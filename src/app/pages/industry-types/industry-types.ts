...
import { IndustryType } from '../../core/models';
...
@Component({
  ...
  styleUrl: './industry-types.css',
})
export class IndustryTypesPage {
  private readonly http = inject(HttpClient);
  private readonly toast = inject(ToastService);

  protected readonly rows = signal<IndustryType[]>([]);
  protected readonly loading = signal(false);
  protected readonly saving = signal(false);
  protected readonly search = signal('');
  protected readonly dialogOpen = signal(false);
  protected readonly editing = signal<IndustryType | null>(null);
  ...
  protected async openEdit(item: IndustryType): Promise<void> {
  ...
  protected async save(): Promise<void> {
    ...
    try {
      ...
      if (this.editing()) {
        await firstValueFrom(this.http.put(`/api/industry-types/${this.editing()!.industryTypeId}`, payload));
        this.toast.success('Industry type updated');
      } else {
        await firstValueFrom(
          this.http.post('/api/industry-types', {
            name: this.form.name().trim(),
            description: this.form.description() || null,
            sortOrder: parseInt(this.form.sortOrder(), 10) || 1,
          }),
        );
        this.toast.success('Industry type created');
      }
  ...
  protected async remove(item: IndustryType): Promise<void> {
    if (!confirm(`Delete industry type "${item.name}"? This cannot be undone.`)) return;
    try {
      await firstValueFrom(this.http.delete(`/api/industry-types/${item.industryTypeId}`));
      this.toast.success('Industry type deleted');
      await this.load();
    } catch {
      /* handled by interceptor */
    }
  ...
  private async load(): Promise<void> {
    ...
    try {
      const res = await firstValueFrom(this.http.get<IndustryType[]>('/api/industry-types?includeInactive=true'));
      this.rows.set(res);
    } catch {
      this.toast.error('Failed to load industry types');
    } finally {
